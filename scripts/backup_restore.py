...
class BackupRestore:
    ...
    def create_backup(self):
        ...
        try:
                            ...
                            if source.is_file():
                                ...
                                for file in source.rglob("*"):
                                    if file.is_file():
                                        ...
                                        manifest["total_files"] += 1
                                print(
                                    f"   ✅ Added directory: {item} ({manifest['total_files']} files)"
                                )
                        else:
            ...
            print(
                f"   Size: {backup_size_mb:.2f} MB (compressed from {size_mb:.2f} MB)"
            )
    ...
    def list_backups(self):
        ...
        for i, backup in enumerate(backups, 1):
            ...
            print(
                f"     Size: {size_mb:.2f} MB | Created: {created.strftime('%Y-%m-%d %H:%M:%S')}{manifest_info}"
            )
            print()
...
Examples:
  ...
  python backup_restore.py verify --backup-file backups/jgmart_backup_20260729.tar.gz
    ...
    backup_parser.add_argument(
        "--no-compress", action="store_true", help="Create uncompressed backup"
    )
    backup_parser.add_argument(
        "--cleanup",
        action="store_true",
    ...
    restore_parser.add_argument(
        "--backup-file", type=str, required=True, help="Path to backup file"
    )
    restore_parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.command == "backup":
        tool = BackupRestore(output_dir=args.output_dir, compress=not args.no_compress)
        ...
        tool = BackupRestore()
        success = tool.restore_backup(args.backup_file, dry_run=args.dry_run)
